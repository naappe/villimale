// ============================================
// PNC TRACKER - SPECIFIC LOGIC
// ============================================

// ============================================
// CONFIG
// ============================================
const PARTY_NAME = 'PNC';
const PARTY_CONFIG = window.PARTY_AUTH ? window.PARTY_AUTH[PARTY_NAME] : null;
const PARTY_PASSWORD = PARTY_CONFIG ? PARTY_CONFIG.password : 'pnc2024';
const PARTY_COLOR = PARTY_CONFIG ? PARTY_CONFIG.color : '#1abc9c';

let allVoters = [];
let filteredVoters = [];
let selectedVoterId = null;
let target = parseInt(localStorage.getItem('pncReachTarget')) || 100;
let currentPage = 1;
const pageSize = 25;
...
function checkPasswordSession() {
    try {
        return localStorage.getItem('pncSession') === 'true';
    } catch (e) {
        return false;
    }
}
...
function savePasswordSession() {
    try {
        localStorage.setItem('pncSession', 'true');
    } catch (e) {
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🔐 PNC Tracker loaded');
    console.log('🔑 Expected password:', PARTY_PASSWORD);
...
// ============================================
// PASSWORD FORM SUBMIT - FIXED
// ============================================
passwordForm.addEventListener('submit', function(e) {
    e.preventDefault();
    const entered = passwordInput.value.trim();
    
    console.log('🔐 Password entered:', entered);
    console.log('🔑 Expected password:', PARTY_PASSWORD);
    ...
    if (entered === PARTY_PASSWORD) {
        ...
        // Hide password overlay, show app
        passwordOverlay.classList.add('hidden');
        mainApp.style.display = 'block';
...
async function loadPNCData() {
    console.log('📊 Loading PNC voters...');
    voterGrid.innerHTML = '<div class="loading-state">Loading PNC voters...</div>';
    ...
    try {
        ...
        console.log('✅ Loaded PNC voters:', data ? data.length : 0);

        if (!data || data.length === 0) {
            voterGrid.innerHTML = `
                <div class="no-results" style="grid-column:1/-1; padding:40px; text-align:center;">
                    <i class="fas fa-database" style="font-size:48px; color:#e8ddd0; margin-bottom:16px; display:block;"></i>
                    <h3 style="color:#1a1a2e; margin-bottom:8px;">No PNC voters found</h3>
                    <p style="color:#888; font-size:14px; margin-bottom:16px;">
                        The PNC voters table is empty. Please add sample data.
                    </p>
                    <button onclick="loadSamplePNCData()" style="padding:10px 24px; background: #1abc9c; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600;">
        ...
        voterGrid.innerHTML = `
            <div class="no-results" style="grid-column:1/-1; padding:40px; text-align:center;">
                ...
                <h3 style="color:#1a1a2e; margin-bottom:8px;">Error loading voters</h3>
                <p style="color:#888; font-size:14px;">${error.message || 'Please check your Supabase connection.'}</p>
                <button onclick="loadPNCData()" style="margin-top:12px; padding:8px 20px; background: #1abc9c; color: white; border: none; border-radius: 6px; cursor: pointer;">
...
// ============================================
// LOAD SAMPLE PNC DATA
// ============================================
async function loadSamplePNCData() {
    const supabaseClient = createSupabaseClient();
    ...
    for (let i = 1; i <= 50; i++) {
        sampleVoters.push({
            name: `PNC Voter ${i}`,
            national_id: `PNC${String(i).padStart(6, '0')}`,
            house: `House ${(i % 10) + 1}`,
            lives_in: 'Male',
            phone: `9${String(i).padStart(6, '0')}`,
            sex: i % 2 === 0 ? 'Male' : 'Female',
            age: String(20 + (i % 50)),
            party: 'PNC',
            image_number: i,
            reach_status: i % 3 === 0 ? 'reached' : i % 3 === 1 ? 'not-reached' : 'pending',
            vote_status: i % 4 === 0 ? 'will-vote' : i % 4 === 1 ? 'not-vote' : 'pending'
        });
    }
    ...
    try {
        ...
        alert('✅ Sample PNC voters loaded successfully!');
        loadPNCData();
...
function setTarget() {
    ...
    if (val > 0 && val <= allVoters.length) {
        target = val;
        localStorage.setItem('pncReachTarget', target);
        updateStats(filteredVoters);
    } else {
...
// ============================================
function exportData() {
    exportVotersToCSV(filteredVoters, 'PNC');
}
...
window.loadSamplePNCData = loadSamplePNCData;
window.loadPNCData = loadPNCData;
...
targetInput.value = target;
console.log('✅ PNC Tracker loaded');
console.log('🔑 Use password: pnc2024');